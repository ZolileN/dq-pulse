...
export function TrendsExplorer() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Trends"
        description="Counts are summed at the selected grain. Rates are computed from those sums — never averaged across periods. Click any indicator to expand."
      />
      ...
      <Tabs defaultValue={defaultTab} className="isolate">
        ...
        {categories.map((cat) => (
          <TabsContent key={cat.dataType} value={cat.dataType} className="mt-0">
            {loading ? (
